...
struct Modular{
    ...
    template<typename T>
    Modular(const T& v): value(v%MOD) { if(value<0) value+=MOD; }

    template<typename T>
    static T inv(T x, T m) { return x>1?m-1LL*inv(m%x,x)*m/x:1; }
    ...
    friend const int abs(const Modular& number) { return number.value; }
